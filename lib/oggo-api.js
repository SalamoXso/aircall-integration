...
const { OGGO_CONTACT_TYPES, OGGO_CONTACT_STATUSES } = require('../config/constants');

class OGGOApi {
  constructor(consumerKey, consumerSecret, baseUrl = 'https://oggo-data.net/api/') {
  ...
  async createContact(contactData) {
    ...
    try {
      ...
      console.error('OGGO API Error:', error.response?.data || error.message);
      throw error;
    }
  }
  ...
  async searchContact(phoneNumber) {
    const url = `${this.baseUrl}contact?filters[phones]=${phoneNumber}`;
